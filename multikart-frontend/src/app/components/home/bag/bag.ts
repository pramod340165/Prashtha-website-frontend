...
import { bagsProduct } from '../../../shared/data/owl-carousel';
import { ICategory } from '../../../shared/interface/category.interface';
import { IBag } from '../../../shared/interface/theme.interface';
...
export class Bag {
  ...
  ngOnChanges() {
    ...
    if (data?.slug == this.slug()) {
      let categoryIds = [
        ...new Set(
          data?.content?.category?.category_ids.concat(data.content.category_product.category_ids),
        ),
      ];
      ...
      if (
        categoryIds.length &&
        (data?.content?.category_product?.status || data?.content?.category?.status)
      ) {
