...
import { IElectronicThree } from '../../../../shared/interface/theme.interface';
...
@Component({
  ...
  templateUrl: './electronic-3.html',
  styleUrl: './electronic-3.scss',
})
export class Electronic3 {
  private store = inject(Store);
  private themeOptionService = inject(ThemeOptionService);

  readonly data = input<IElectronicThree>();
  ...
  ngOnChanges() {
    ...
    if (data?.slug == this.slug()) {
      let categoryIds = [
        ...new Set(
          data?.content?.category_product_1?.categories?.category_ids.concat(
